...
impl TrayManager {
    ...
    pub fn initialize<R: Runtime>(&self, app_handle: &AppHandle<R>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let menu = match self.build_menu(true) {
            Ok(menu) => menu,
            Err(e) => {
    ...
    fn build_menu<R: Runtime>(&self, expansion_enabled: bool) -> Result<Menu<R>, Box<dyn std::error::Error>> {
        ...
        let toggle_item = MenuItem::with_id("toggle_expansions", toggle_text, true, None::<&str>)?;
        let separator1 = MenuItem::separator()?;
        let open_item = MenuItem::with_id("open_window", "Open DotDashDash", true, None::<&str>)?;
        let diagnostics_item = MenuItem::with_id("diagnostics", "Diagnostics", true, None::<&str>)?;
        let separator2 = MenuItem::separator()?;
        let quit_item = MenuItem::with_id("quit", "Quit DotDashDash", true, None::<&str>)?;
    ...
    fn update_menu<R: Runtime>(&self, app_handle: &AppHandle<R>, expansion_enabled: bool) -> Result<(), Box<dyn std::error::Error>> {
        let menu = self.build_menu(expansion_enabled)?;
...
pub fn handle_tray_menu_event(app_handle: &AppHandle, event_id: &str) {
...
fn toggle_pause_via_tray(app_handle: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
...
fn cleanup_before_quit(app_handle: &AppHandle) {
